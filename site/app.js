(() => {
  ...
  const screens = {
    inspect: {alt:'新版 UIDelta 测量面板：在网页上查看尺寸、布局、间距和外观属性',description:'选中元素，查看尺寸、间距与样式，在当前页面试改并验证设计方向。'},
    record: {alt:'新版 UIDelta 记录面板：页面全景、元素细节、问题描述与展开的分类选项',description:'写下修改要求，选择类型、优先级与影响程度。页面全景、元素细节和定位随记录保存。'},
    delivery: {alt:'新版 UIDelta 交付面板：HTML、XLSX、ZIP 和悬浮示例入口',description:'HTML 查看跟进，XLSX 整理排期，ZIP 交给前端或 Agent。导出前可悬停「示例」查看用途。'}
  };
